...
import { Download, Link, Copy, Check, Video, Loader2, AlertCircle, ExternalLink, Instagram, Heart, MessageCircle } from "lucide-react";
...
export default function InstagramVideoDownloader() {
  ...
  const [postInfo, setPostInfo] = useState<{ username: string; likes: string; comments: string; caption: string } | null>(null);
    ...
    <div className="p-6 md:p-8 space-y-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-black text-slate-900 uppercase italic">Instagram Video Downloader</h2>
          ...
          <div className="p-6">
            ...
            <div className="relative">
              ...
              <Link className={`absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 ${
                isValid === true ? "text-emerald-500" : isValid === false ? "text-red-500" : "text-slate-400"
              }`} />
            </div>
            {isValid === false && (
            ...
            <div className="px-6 pb-6 space-y-6">
              ...
              <div className="bg-slate-50 rounded-2xl p-6">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Video Preview</span>
                  </div>
                  <button onClick={openInstagram} className="flex items-center gap-2 text-sm text-pink-600 hover:text-pink-700 font-medium">
                ...
                <div className="mt-4 space-y-2">
                  <p className="font-bold text-slate-800">@{postInfo.username}</p>
                  <p className="text-sm text-slate-600 line-clamp-2">{postInfo.caption}</p>
                  ...
                  {qualityOptions.map((quality) => (
                    <button
                      ...
                      className={`p-4 rounded-xl border-2 transition-all ${
                        selectedQuality.resolution === quality.resolution ? "border-pink-500 bg-pink-50" : "border-slate-200 hover:border-slate-300"
                      }`}
                    >
              ...
              <div className="flex flex-wrap gap-4">
                <button
                  ...
                  disabled={isLoading}
                  className="flex-1 min-w-[150px] h-14 bg-gradient-to-r from-pink-500 to-purple-600 text-white rounded-2xl font-bold flex items-center justify-center gap-2 hover:from-pink-600 hover:to-purple-700 transition-all shadow-lg shadow-pink-100 disabled:opacity-50"
                >
                  {isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : <Download className="h-5 w-5" />}
                  {isLoading ? "Processing..." : `Download ${selectedQuality.resolution}`}
                </button>
                <button
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="p-4 bg-white rounded-2xl border border-slate-100 shadow-sm">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 bg-pink-100 rounded-xl flex items-center justify-center"><Video className="h-5 w-5 text-pink-600" /></div>
              <div><h4 className="font-bold text-slate-800 text-sm">HD Quality</h4><p className="text-xs text-slate-500">Up to 1080p</p></div>
            </div>
          </div>
          <div className="p-4 bg-white rounded-2xl border border-slate-100 shadow-sm">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 bg-purple-100 rounded-xl flex items-center justify-center"><Download className="h-5 w-5 text-purple-600" /></div>
              <div><h4 className="font-bold text-slate-800 text-sm">Fast Download</h4><p className="text-xs text-slate-500">Quick processing</p></div>
            </div>
          </div>
          <div className="p-4 bg-white rounded-2xl border border-slate-100 shadow-sm">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 bg-orange-100 rounded-xl flex items-center justify-center"><Instagram className="h-5 w-5 text-orange-600" /></div>
              <div><h4 className="font-bold text-slate-800 text-sm">All Videos</h4><p className="text-xs text-slate-500">Reels, IGTV, Posts</p></div>
            </div>
          </div>
        </div>
      </div>
